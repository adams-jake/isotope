	...
	public function isotope_loop( $atts ) {
      ...
      ob_start();
		...
		if ( $filter_menu == 'yes' && taxonomy_exists( $filter_by ) ) {
         ...
         if ( $count > 0 ) {
				echo '<ul id="filters" cursor-class="invert">';
				echo '<li class="active"><a href="#" data-filter="*">' . __('See All', 'isotope-posts-locale') . '</a></li>';

            foreach ( $terms as $term ) {
					echo '<li><a href="#'. $term->slug .'" class="filter-button filter-'. $term->slug .'" data-filter=".'. $term->slug .'">' . $term->name . '</a></li>';
				}
				echo '</ul>';
			}
   			...
   			<?php while ( $isoposts->have_posts() ) : $isoposts->the_post(); ?>
   				<li cursor-class="invert" class="<?php if ( $filter_menu == 'yes' && taxonomy_exists( $filter_by ) ) {
					...
					<a href="<?php the_permalink(); ?>">
						<img src="<?php the_post_thumbnail_url('thumbnail'); ?>" alt="Axel Kacoutie - <?php the_title();?> - Work Thumbnail">
						<div class=""></div>
		...
		<div class="nothing-found">
			<h4 class="seen-everything">You’ve seen <span class="call-to-action">everything!</span></h4>
			<p>Be sure to check back soon to see my latest pieces</p>
		</div>
